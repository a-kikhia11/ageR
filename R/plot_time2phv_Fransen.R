#' Time to PHV Dumbbell Plot (Fransen)
#'
#' This function returns a dumbbell plot showing the difference (in years) between current age and estimated age at PHV for each athlete in the dataset using the Fransen Maturity Offset method. Can be further filtered by athlete, age, and date.
#'
...
plot_time2phv_Fransen <- function(data, athlete = NULL, date = NULL, agegroup = NULL) {
  ...
  datos <- data %>%
    dplyr::select(`Player Name`, `Current Age` = Age, `Age @ PHV (Fransen)`)

  base_diff <- datos %>%
    dplyr::mutate(Difference = `Age @ PHV (Fransen)` - `Current Age`) %>%
  ...
  plot <-  ggplot2::ggplot(data = df_diff_gather_age, ggplot2::aes( y = Athlete.fact, x = Value, color = Metric)) +
    ...
    ggplot2::scale_color_manual(name="Time", values = c("Current Age" = "deepskyblue4", "Age @ PHV (Fransen)" = "deepskyblue")) +
